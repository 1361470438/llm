...
前端在 [`js/app.js`](file:///c:/Users/a1361/Desktop/llm-chat/js/app.js) 中实现了**三层容灾、免费优先与 6 秒超时降级**机制：
...
4. **免费网关 6 秒智能超时降级 (`tryProxyGateways`)**：
   * **优先打向免费 Cloudflare 网关**（如 `api1.yulucha.xyz` 或本站同源 `/api/proxy`），最大限度节省云函数调用。
   * **首包 6 秒超时熔断**：如果企业内网对 Cloudflare 发生丢包、阻断导致 6 秒内无法建立连接或返回首包，自动触发 `AbortController` 熔断。
   * **无缝切换至国内阿里云**：超时或遭遇 502/503/网络错误后，前端控制台打印警告并**立刻、静默转交国内阿里云函数**（10ms 直达），全过程用户无感知，兼顾“省钱”与“高可用”。
   * **长文本流不受限**：只要首包握手成功开始吐字，超时计时器立刻被注销，大模型持续输出长回答绝不会被截断。
