...
4. **免费网关 OPTIONS 轻量探活与长文本解耦 (`tryProxyGateways`)**：
   * **物理链路快速探活 (`OPTIONS`)**：在发送庞大的大模型 POST 请求前，先对免费 Cloudflare 网关发送轻量级 `OPTIONS` 探活包（3 秒超时）。
     * **网关可达（耗时约 200~600ms）**：说明当前网络能通 Cloudflare，立刻放行执行真实 `POST`。
     * **网关被墙/断连（超时 3 秒）**：说明当前公司内网对 CF 严重丢包，控制台打印警告并**立刻转交下一备用网关（国内阿里云函数）**。
   * **彻底解耦大模型生成时长**：探活通过后，发送真实的 `POST` 业务请求，**不再对大模型输出设置几秒的粗暴短超时**。即便深度思考模型（如 DeepSeek-R1 / o1）需要思考 20~30 秒才开始吐字，网络连接也不会被误杀切换。
